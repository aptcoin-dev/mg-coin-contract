...
/**
 * AptCoin contract learning test
 *   deploy in before, and no deploy between it()
 */
contract("AptCoin", (accounts) => {
  ...
  let NoOfTokensInMinunit, OneAptCoinInMinunit, NoOfTokensInApt;
  let coin
  ...
  before(async () => {
    ...
    OneAptCoinInMinunit = BIG(await coin.getOneAptCoin());
    NoOfTokensInApt = NoOfTokensInMinunit.div(OneAptCoinInMinunit)
  });
  ...
  it("coin initial values", async () => {
    ...
    assert.equal(await coin.symbol(), "APT");
    assert.equal(await coin.name(), "APTCoin");
    assert.equal(await coin.decimals(), 9);
    assert.equal((await coin.totalSupply()).toString(), BIG(NOOFTOKENS).mul(OneAptCoinInMinunit).toString());
  ...
  it("token distribution", async () => {
    ...
    await coin.transfer(admin, OneAptCoinInMinunit, {from: vault});
    assert.equal(await balanceOf(admin), OneAptCoinInMinunit.toString());

    // transfer 0.5 token from admin to owner
    let halfApt = OneAptCoinInMinunit.div(BIG(2))
    await coin.transfer(owner, halfApt, {from: admin});
    assert.equal(await balanceOf(owner), halfApt.toString());
    assert.equal(await balanceOf(admin), halfApt.toString());
    assert.equal(await balanceOf(vault), (NoOfTokensInApt.sub(BIG(1)).mul(OneAptCoinInMinunit).toString()));
  });
  ...
  it("marginal transfers", async () => {
    ...
    try {
      await coin.transfer(user2, OneAptCoinInMinunit.mul(BIG(1)), {from: user1});
      assert.fail();
    }
    ...
    try
    {
      await coin.transfer(user2, OneAptCoinInMinunit.mul(BIG(0)), {from: user1});
      assert.equal(await balanceOf(user1), OneAptCoinInMinunit.mul(BIG(0)).toString());
      assert.equal(await balanceOf(user2), OneAptCoinInMinunit.mul(BIG(0)).toString());
    }
    catch(exception) { assert.fail(); }

    // transfer 0 token from vault(balance > 0) to user1
    try {
        await coin.transfer(user1, OneAptCoinInMinunit.mul(BIG(0)), {from: vault});
        assert.equal(await balanceOf(user1), OneAptCoinInMinunit.mul(BIG(0)).toString());
    } catch(exception) { assert.fail(); }

    // transfer whole balance to other
    await coin.transfer(user1, OneAptCoinInMinunit.mul(BIG(1)), {from: vault});
    assert.equal(await balanceOf(user1), OneAptCoinInMinunit.mul(BIG(1)).toString());
    await coin.transfer(user2, OneAptCoinInMinunit.mul(BIG(1)), {from: user1});
    assert.equal(await balanceOf(user1), OneAptCoinInMinunit.mul(BIG(0)).toString());
    assert.equal(await balanceOf(user2), OneAptCoinInMinunit.mul(BIG(1)).toString());
  });
  ...
  it("only admin can set reserve", async () => {
    await coin.setReserve(user1, OneAptCoinInMinunit, {from: admin});
  ...
  it("reserve and reserveOf", async () => {
    ...
    await coin.transfer(user1, OneAptCoinInMinunit.mul(BIG(10)), { from: vault });
    await coin.setReserve(user1, OneAptCoinInMinunit.mul(BIG(5)), { from: admin });
    assert.equal(await reserveOf(user1), OneAptCoinInMinunit.mul(BIG(5)).toString());
  });
  
  it("transfer after setting reservation", async () => {
    // user1 = 10 Apt
    await coin.transfer(user1, OneAptCoinInMinunit.mul(BIG(10)), { from: vault });
    // user1 = 10 Apt with 5 Apt reserve
    await coin.setReserve(user1, OneAptCoinInMinunit.mul(BIG(5)), { from: admin });
    // user1 = 9 Apt with 5 Apt reserve
    await coin.transfer(user2, OneAptCoinInMinunit.mul(BIG(1)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneAptCoinInMinunit.mul(BIG(9)).toString());

    // user1 = 6 Apt with 5 Apt reserve
    coin.transfer(user2, OneAptCoinInMinunit.mul(BIG(3)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneAptCoinInMinunit.mul(BIG(6)));

    try {
      let tx = await coin.transfer(user2, OneAptCoinInMinunit.mul(BIG(2)), {from: user1}); // violating reserved amount
      assert.equal(await balanceOf(user1), OneAptCoinInMinunit.mul(BIG(6)));
      assert.fail();
    }
    catch(exception) {
