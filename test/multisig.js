...
const AptCoin = artifacts.require('AptCoin.sol');
const AptCoinMultiSigFactory = artifacts.require('AptCoinMultiSigWallet.sol');
const web3 = AptCoin.web3;
const eth = web3.eth;
...
const deployMultiSigWallet = (owners, requiredConfirm, coin) => {
  return AptCoinMultiSigFactory.new(owners, requiredConfirm, coin, "vault");
};

const deployAptCoin = (owner,admin,vault) => {
  return AptCoin.new("Apt", "AptCoin", owner, admin, vault);
};
...
contract("AptCoinMultiSigWallet", (accounts) => {
  const BIG = (v) => new theBN.BN(v)
  let coinInstance;
  let walletInstance;
  const requiredConfirmations = 2;

  let OneAptCoinInMinunit;
  ...
  beforeEach(async () => {
    coinInstance = await deployAptCoin(owner, admin, vault);
    assert.ok(coinInstance);

    NoOfTokensInMinunit = await coinInstance.totalSupply();
    OneAptCoinInMinunit = await coinInstance.getOneAptCoin();
  ...
  it("multisig wallet basic functions", async () => {
    assert.equal( await walletInstance.aptCoin(), coinInstance.address );
  ...
  it("only signers can request TX", async () => {
    // user1 cannot
    assertRevert("1-1",() => walletInstance.requestTransferInApt(user1, 1, 0, "test 1", {from: user1}));
    ...
    // owner cannot
    assertRevert("2-1",() => walletInstance.requestTransferInApt(user1, 1, 0, "test 2-1", {from: owner}));
    ...
    // admin(as signer) can
    assertNotRevert("3-1",() => walletInstance.requestTransferInApt(user1, 1, 0, "test 3-1", {from: admin}));
    ...
    // vault(as signer) can
    assertNotRevert("4-1",() => walletInstance.requestTransferInApt(user1, 1, 0, "test 4-1", {from: vault}));
    ...
    assertNotRevert("5-1",() => walletInstance.requestTransferInApt(user1, 1, 0, "test 5-1", {from: signer}));
  ...
  it("signers only methods", async () => {
    // request a TX
    walletInstance.requestTransferInApt(user1, 1, 0, "test 1", {from: signer});
  ...
  it("test transfer using multisig wallet", async () => {
    ...
    const thousandAptInMinunit = OneAptCoinInMinunit.mul(BIG(1000))
    coinInstance.transfer(walletInstance.address, thousandAptInMinunit, {from:vault});
    assert.equal(await balanceOf(walletInstance.address), thousandAptInMinunit.toString());

    // Add aptcoin transfer transaction
    // example ABI generation:
    // const transferData = coinInstance.contract.transfer.getData(user1, OneAptCoin * 1);
    //

    const MSG1 = "send 1Apt and 1DENNIS to user1";
    const trSubmit = await walletInstance.requestTransferInApt(user1, 1, 1, MSG1, {from:vault});
    ...
    assert.equal(afterBalance.toString(), OneAptCoinInMinunit.mul(BIG(1)).add(BIG(1)).toString());
  ...
  it("Add a signer", async () => {
    ...
    assertNotRevert("signer1",() => walletInstance.requestTransferInApt(user2, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Remove a signer", async () => {
    ...
    assertRevert("signer1",() => walletInstance.requestTransferInApt(user1, 1, 0, "test addsigner-1", {from: user1}));
